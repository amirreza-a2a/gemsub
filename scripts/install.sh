#!/usr/bin/env bash
# scripts/install.sh — Production-grade Linux installer for gemsub
#
# Default behavior:
#   Installs gemsub to ~/.local/bin (no root required)
#
# Optional system-wide behavior:
#   --system or SYSTEM_INSTALL=1: Installs to /usr/local/bin
#
# Package-managed note:
#   Official .deb and .rpm packages install to /usr/bin/gemsub
#
...
#   OS_OVERRIDE: simulate operating system
    ...
    case "$os" in
        Linux|linux)
            ...
            if [ -n "${TERMUX_VERSION:-}" ] || [ -d "/data/data/com.termux" ]; then
                log_error "Detected Termux on Android. Please use 'scripts/install-termux.sh' instead."
                return 1
            fi
            echo "Linux"
            ;;
        *)
            log_error "Unsupported operating system '$os'. This installer is only for Linux."
            return 1
            ;;
    esac
}
...
detect_arch() {
    ...
    case "$machine" in
        ...
        armv7l|armv7|armhf)
            ...
            ;;
        *)
            log_error "Unsupported architecture '$machine'. Supported: x86_64, arm64, armv7."
            return 1
            ;;
    esac
}
...
main() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            -v|--version)
                ...
                ;;
            -h|--help)
                echo "Usage: $0 [--system] [-v <version>]"
                echo ""
                echo "Options:"
                echo "  --system         Install system-wide to /usr/local/bin (requires write or sudo)"
    ...
    elif [ "$system_mode" -eq 1 ] || [ "${SYSTEM_INSTALL:-0}" = "1" ]; then
    ...
    log_info "Target release: ${tag} (${arch})"

    # 5. Prepare temporary workspace
    TMP_DIR=$(mktemp -d -t gemsub-install.XXXXXX 2>/dev/null || mktemp -d)

    local archive_name="gemsub_Linux_${arch}.tar.gz"
    ...
    log_info "Downloading release artifact from ${archive_url}..."
    ...
    # 9. Atomically install executable to target
    log_info "Installing gemsub to ${install_dir}..."
    mkdir -p "$install_dir" 2>/dev/null || {
        if [ "$system_mode" -eq 1 ] && command -v sudo >/dev/null 2>&1; then
            log_info "Root privileges required for ${install_dir}. Requesting sudo..."
            sudo mkdir -p "$install_dir"
        else
            log_error "Permission denied creating directory ${install_dir}. Run with sudo or omit --system."
            exit 1
        fi
    }
    ...
    if [ -w "$install_dir" ]; then
        ...
        log_info "Root privileges required to install to ${install_dir}. Requesting sudo..."
        sudo cp "${TMP_DIR}/gemsub" "${TEMP_BIN}"
        sudo chmod 755 "${TEMP_BIN}"
        sudo mv -f "${TEMP_BIN}" "${target_bin}"
        TEMP_BIN=""
    else
        log_error "Cannot write to ${install_dir}. Run with sudo or install to user directory."
        exit 1
    fi

    log_info "Successfully installed gemsub to ${target_bin}"

    # 10. Check PATH notification
    case ":${PATH}:" in
        *":${install_dir}:"*) ;;
        *)
            echo ""
            echo "Note: '${install_dir}' is not currently in your PATH."
            echo "To make gemsub available globally, add this to your shell configuration:"
